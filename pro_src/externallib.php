...
class block_use_stats_external_extended extends external_api {
    ...
    public static function get_user_stats($uidsource, $uid, $cidsource, $cid, $from, $to, $score = 0) {
        ...
        if (array_key_exists('course', $aggregate)) {
            ...
            foreach (array_keys($aggregate['course']) as $courseid) {
                ...
                $courseres->activitytime = 0 + @$aggregate['activities'][$courseid]->elapsed;
                ...
                if ($score) {
                    if ($gradeitem = $DB->get_record('grade_items', array('itemtype' => 'course', 'courseid' => $courseid))) {
    ...
    public static function get_users_course_stats($uidsource, $uids, $cidsource, $cid, $from, $to, $score) {
        ...
        foreach ($uids as $uid) {
            ...
            $result[] = self::get_user_course_stats($uidsource, $uid, $cidsource, $cid, $from, $to, $score);
        }
    ...
    protected static function get_user_course_stats($uidsource, $uid, $cidsource, $cid, $from, $to, $score = 0) {
        global $DB;
        ...
        */
        ...
        if (!empty($logs)) {
            $aggregate = use_stats_aggregate_logs($logs, $from, $to, '', false, $course);
        }

        $courseres = new StdClass;

        if (!empty($logs) && array_key_exists('course', $aggregate)) {

            $courseres->id = $course->id;
            $courseres->shortname = $course->shortname;
            $courseres->idnumber = $course->idnumber;
            $courseres->fullname = format_string($course->fullname);

            $courseres->activitytime = 0 + @$aggregate['activities'][$course->id]->elapsed;
            $courseres->coursetime = $aggregate['course'][$course->id]->elapsed;
            $courseres->coursetotal = $aggregate['coursetotal'][$course->id]->elapsed;
            $courseres->othertime = 0 + @$aggregate['other']->elapsed;
            $courseres->sitecoursetime = 0 + @$aggregate['course'][SITEID]->elapsed;
            if (array_key_exists('sessions', $aggregate)) {
                $firstkey = array_shift(array_keys($aggregate['sessions']));
                $firstsession = $aggregate['sessions'][$firstkey];
                $courseres->firstsession = 0 + $firstsession->sessionstart;
                $lastkey = array_pop(array_keys($aggregate['sessions']));
                $lastsession = $aggregate['sessions'][$lastkey];
                $courseres->lastsession = 0 + $lastsession->sessionstart;
            }

            if ($score) {
                if ($gradeitem = $DB->get_record('grade_items', array('itemtype' => 'course', 'courseid' => $course->id))) {
                    $grade = $DB->get_record('grade_grades', array('itemid' => $gradeitem->id, 'userid' => $user->id));
                    if ($grade) {
                        $courseres->score = $grade->rawgrade;
                    } else {
            ...
            $courseres->activitytime = 0;
            $courseres->coursetime = 0;
            $courseres->coursetotal = 0;
            $courseres->othertime = 0;
            $courseres->sitecoursetime = 0;
            $courseres->firstsession = 0;
            $courseres->lastsession = 0;
        }
        $result->coursedata = $courseres;
