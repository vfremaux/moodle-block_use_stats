...
if (!$cansee) {
    throw new moodle_exception(get_string('notallowed', 'block_use_stats'));
}

// M4.
$fields = \core_user\fields::for_name()->with_userpic()->get_required_fields();
$user = $DB->get_record('user', array('id' => $userid), implode(',', $fields));
...
$PAGE->set_url(new moodle_url('/blocks/use_stats/pro/detail.php', $params));
...
$aggregate = use_stats_aggregate_logs($logs, $fromwhen, $towhen);
